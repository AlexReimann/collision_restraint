...
std::shared_ptr<Params> params = std::make_shared<Params>(-1.0F, 1.0F, false, 0.0F);
...
TEST_CASE("setVelocities", "[distance_model]")
{
  ...
  SECTION("straight")
  {
    ...
    CHECK(model.radii().inner_ == std::numeric_limits<float>::infinity());
    CHECK(model.radii().center_ == std::numeric_limits<float>::infinity());
    CHECK(model.radii().outer_ == std::numeric_limits<float>::infinity());
  ...
  SECTION("left turn")
  {
    ...
    CHECK(model.radii().inner_ == (1.0F - half_width));
    CHECK(model.radii().center_ == 1.0F);
    CHECK(model.radii().outer_ == Catch::Approx(std::sqrt(2.0) * front_offset));
  }
  ...
  SECTION("zero linear velocity")
  {
    ...
    CHECK(model.radii().inner_ == 0.0F);
    CHECK(model.radii().center_ == 0.0F);
    CHECK(
      model.radii().outer_ == std::sqrt((front_offset * front_offset) + (half_width * half_width)));
  }
  ...
  SECTION("back offset > front offset")
  {
    ...
    CHECK(long_back.radii().inner_ == (1.0F - half_width));
    CHECK(long_back.radii().center_ == 1.0F);
    CHECK(long_back.radii().outer_ == Catch::Approx(std::sqrt(2.0) * front_offset));

    long_back.setVelocities(0.0F, 1.0F);
    CHECK(
      long_back.radii().outer_ ==
